...
if __name__ == '__main__':
    ...
    urls_list = extract_all_urls(root=root_url, page_stop=None) # Download from all pages
    print(f"Extracted {len(urls_list)} URLs.")

    # Create directories if they don't exist
    Path("data/blog_posts/json").mkdir(parents=True, exist_ok=True)
    ...
    for url in tqdm(urls_list):
        try:
            ...
            if response:
                soup = BeautifulSoup(response.content, "html.parser")
                blog_data = extract_blog_data(soup, url)
                # Save blog data to JSON file
                url_tail = url.replace(root_url, "").replace("/", "")
                file_path = Path("data/blog_posts/json") / f"{url_tail}.json"
                with open(file_path, "w", encoding="utf-8") as json_file:
                    json.dump(blog_data, json_file, ensure_ascii=True, indent=4)
                print(f"Successfully scraped and saved data from {url}: {blog_data['title']}")
            else:
